...
public class SqlSessionFactoryBean
  ...
  /**
   * Set locations of MyBatis com.mapper files that are going to be merged into the {@code SqlSessionFactory}
   * configuration at runtime.
   *
   * This is an alternative to specifying "&lt;sqlmapper&gt;" entries in an MyBatis config file. This property being
   * based on Spring's resource abstraction also allows for specifying resource patterns here: e.g.
   * "classpath*:sqlmap/*-com.mapper.xml".
   *
   * @param mapperLocations
   *          location of MyBatis com.mapper files
   */
  ...
  protected SqlSessionFactory buildSqlSessionFactory() throws Exception {
    ...
    if (this.databaseIdProvider != null) {// fix #64 set databaseId before parse com.mapper xmls
      try {
    ...
    if (this.mapperLocations != null) {
      if (this.mapperLocations.length == 0) {
        ...
        for (Resource mapperLocation : this.mapperLocations) {
          ...
          LOGGER.debug(() -> "Parsed com.mapper file: '" + mapperLocation + "'");
        }
      }
    } else {
